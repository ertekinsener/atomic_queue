...
template <typename T, typename A = std::allocator<T>, T NIL = details::nil<T>(),
...
class AtomicQueueB
    : private std::allocator_traits<A>::template rebind_alloc<std::atomic<T>>,
      public AtomicQueueCommon<
          AtomicQueueB<T, A, NIL, MAXIMIZE_THROUGHPUT, TOTAL_ORDER, SPSC>> {
  using AllocatorElements =
      typename std::allocator_traits<A>::template rebind_alloc<std::atomic<T>>;
  ...
  static constexpr auto ELEMENTS_PER_CACHE_LINE =
      CACHE_LINE_SIZE / sizeof(std::atomic<T>);
  static_assert(ELEMENTS_PER_CACHE_LINE, "Unexpected ELEMENTS_PER_CACHE_LINE.");

  static constexpr auto SHUFFLE_BITS =
      details::GetCacheLineIndexBits<ELEMENTS_PER_CACHE_LINE>::value;
  ...
  // Allocate these immutable members on another cache line which never gets
  // invalidated by stores.
  alignas(CACHE_LINE_SIZE) unsigned size_;

  // The C++ strict aliasing rules assume that pointers to the same decayed type
  // may alias. The C++ strict aliasing rules assume that pointers to any char
  // type may alias anything and everything. A dynamically allocated array may
  // not alias anything else by construction. Explicitly annotate the circular
  // buffer array pointer as not aliasing anything else with restrict keyword.
 ...
 public:
  ...
  AtomicQueueB(unsigned size, A const& allocator = A{})
      : AllocatorElements(allocator),
        size_(max_value(details::round_up_to_power_of_2(size),
                        1u << (SHUFFLE_BITS * 2))),
    ...
    assert(get_allocator() ==
           allocator);  // The standard requires the original and rebound
                        // allocators to manage the same state.
  }

  AtomicQueueB(AtomicQueueB&& b) noexcept
      : AllocatorElements(static_cast<AllocatorElements&&>(
            b))  // TODO: This must be noexcept, static_assert that.
        ,
  ...
  ~AtomicQueueB() noexcept {
    if (elements_) {
      ...
      AllocatorElements::deallocate(
          elements_,
          size_);  // TODO: This must be noexcept, static_assert that.
    }
  }

  A get_allocator() const noexcept {
    return *this;  // The standard requires implicit conversion between rebound
                   // allocators.
  }

  void swap(AtomicQueueB& b) noexcept {
    using std::swap;
    swap(static_cast<AllocatorElements&>(*this),
         static_cast<AllocatorElements&>(b));
    Base::swap(b);
    swap(size_, b.size_);
    swap(elements_, b.elements_);
  }

  ATOMIC_QUEUE_INLINE friend void swap(AtomicQueueB& a,
...
class AtomicQueueB2
    : private std::allocator_traits<A>::template rebind_alloc<unsigned char>,
      public AtomicQueueCommon<
          AtomicQueueB2<T, A, MAXIMIZE_THROUGHPUT, TOTAL_ORDER, SPSC>> {
  ...
  // type may alias anything and everything. A dynamically allocated array may
  // not alias anything else by construction. Explicitly annotate the circular
  // buffer array pointers as not aliasing anything else with restrict keyword.
  AtomicState* ATOMIC_QUEUE_RESTRICT states_;
  T* ATOMIC_QUEUE_RESTRICT elements_;

  static constexpr auto STATES_PER_CACHE_LINE =
      CACHE_LINE_SIZE / sizeof(AtomicState);
  static_assert(STATES_PER_CACHE_LINE, "Unexpected STATES_PER_CACHE_LINE.");

  static constexpr auto SHUFFLE_BITS =
      details::GetCacheLineIndexBits<STATES_PER_CACHE_LINE>::value;
  static_assert(SHUFFLE_BITS, "Unexpected SHUFFLE_BITS.");
  using B = details::IndexBits<SHUFFLE_BITS>;
  ...
  U* allocate_() {
    ...
    assert(is_suitably_aligned(
        p));  // Allocated storage must be suitably aligned for U.
    return p;
  }

  template <typename U>
  void deallocate_(U* p) noexcept {
    StorageAllocator::deallocate(
        reinterpret_cast<unsigned char*>(p),
        size_ * sizeof(U));  // TODO: This must be noexcept, static_assert that.
  }
 ...
 public:
  ...
  AtomicQueueB2(unsigned size, A const& allocator = A{})
    ...
    A a = get_allocator();
    assert(a == allocator);  // The standard requires the original and rebound
                             // allocators to manage the same state.
  ...
  AtomicQueueB2(AtomicQueueB2&& b) noexcept
      : StorageAllocator(static_cast<StorageAllocator&&>(
            b))  // TODO: This must be noexcept, static_assert that.
        ,
        Base(static_cast<Base&&>(b)),
        size_(std::exchange(b.size_, 0)),
  ...
  void swap(AtomicQueueB2& b) noexcept {
    using std::swap;
    swap(static_cast<StorageAllocator&>(*this),
         static_cast<StorageAllocator&>(b));
    Base::swap(b);
    swap(size_, b.size_);
  ...
  ATOMIC_QUEUE_INLINE friend void swap(AtomicQueueB2& a,
                                       AtomicQueueB2& b) noexcept {
    a.swap(b);
  }
};
