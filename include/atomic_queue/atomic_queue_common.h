...
#if __cpp_lib_atomic_is_always_lock_free  // Better compile-time error message
                                          // requires C++17.
...
template <typename T>
ATOMIC_QUEUE_SINLINE void destroy_n(T* ATOMIC_QUEUE_RESTRICT p,
                                    unsigned n) noexcept {
  for (auto q = p + n; p != q;) (p++)->~T();
}

template <typename T>
ATOMIC_QUEUE_SINLINE void swap_relaxed(std::atomic<T>& a,
...
template <typename T>
ATOMIC_QUEUE_SINLINE void copy_relaxed(std::atomic<T>& a,
...
enum StateE : State { EMPTY, STORED = 1, STORING = 2, LOADING = 4 };

template <typename Derived>
class AtomicQueueCommon {
  ATOMIC_QUEUE_INLINE constexpr auto& downcast() noexcept {
    return static_cast<Derived&>(*this);
  }
  ATOMIC_QUEUE_INLINE constexpr auto& downcast() const noexcept {
    return static_cast<Derived const&>(*this);
  }

 protected:
  // Put these on different cache lines to avoid false sharing between readers
  // and writers.
  ...
  AtomicQueueCommon() noexcept { assert(is_suitably_aligned(&downcast())); }
  ...
  template <typename T>
  ATOMIC_QUEUE_SINLINE T do_pop(std::atomic<T>* ATOMIC_QUEUE_RESTRICT elements,
    ...
    if (Derived::spsc_) {
      ...
      for (;;) {
        ...
        if (ATOMIC_QUEUE_LIKELY(element != NIL)) break;
        // Do speculative loads while busy-waiting to avoid broadcasting RFO
        // messages.
        do spin_loop_pause();
  ...
  template <typename T>
  ATOMIC_QUEUE_SINLINE void do_push(
    ...
    if (Derived::spsc_) {
      while (ATOMIC_QUEUE_UNLIKELY(
          q_element.load(A) !=
          NIL))  // Hint the branch as not taken when the queue is not full.
        if (Derived::maximize_throughput_) spin_loop_pause();
      q_element.store(element, R);
    } else {
      T expected;
      while (ATOMIC_QUEUE_UNLIKELY(!q_element.compare_exchange_weak(
          (expected = NIL), element, AR,
          X)))  // Hint the branch as not taken when the queue is not full.
        do  // Do speculative loads while busy-waiting to avoid broadcasting RFO
            // messages.
  ...
  template <typename T>
  ATOMIC_QUEUE_SINLINE T
  do_pop(std::atomic<State>* ATOMIC_QUEUE_RESTRICT states,
    ...
    if (Derived::spsc_) {
      while (ATOMIC_QUEUE_UNLIKELY(
          state.load(A) !=
          STORED))  // Hint the branch as not taken when the queue is not empty.
        if (Derived::maximize_throughput_) spin_loop_pause();
    } else {
      State expected, desired = LOADING;
      ATOMIC_QUEUE_LEAN_REG(desired);
      while (ATOMIC_QUEUE_UNLIKELY(!state.compare_exchange_weak(
          (expected = STORED), desired, A,
          X))) {  // Hint the branch as not taken when the queue is not empty.
        do  // Do speculative loads while busy-waiting to avoid broadcasting RFO
            // messages.
          spin_loop_pause();
        while (ATOMIC_QUEUE_UNLIKELY(Derived::maximize_throughput_ &&
  ...
  ATOMIC_QUEUE_SINLINE void do_push(
    ...
    if (Derived::spsc_) {
      while (ATOMIC_QUEUE_UNLIKELY(
          state.load(A) !=
          EMPTY))  // Hint the branch as not taken when the queue is not full.
        if (Derived::maximize_throughput_) spin_loop_pause();
    } else {
      State expected, desired = STORING;
      ATOMIC_QUEUE_LEAN_REG(desired);
      while (ATOMIC_QUEUE_UNLIKELY(!state.compare_exchange_weak(
          (expected = EMPTY), desired, A,
          X))) {  // Hint the branch as not taken when the queue is not full.
        do  // Do speculative loads while busy-waiting to avoid broadcasting RFO
            // messages.
          spin_loop_pause();
 ...
 public:
  ...
  ATOMIC_QUEUE_INLINE bool try_push(T&& element) noexcept {
    ...
    if (Derived::spsc_) {
      if (ATOMIC_QUEUE_UNLIKELY(as_signed(head - tail_.load(X)) >=
                                as_signed(downcast().size_)))
        return false;
      ...
      do {
        if (ATOMIC_QUEUE_UNLIKELY(as_signed(head - tail_.load(X)) >=
                                  as_signed(downcast().size_)))
          return false;
      } while (ATOMIC_QUEUE_UNLIKELY(!head_.compare_exchange_weak(
          head, head + 1, X, X)));  // This loop is not FIFO.
    }
  ...
  ATOMIC_QUEUE_INLINE bool try_pop(T& element) noexcept {
    ...
    if (Derived::spsc_) {
      ...
      do {
        if (ATOMIC_QUEUE_UNLIKELY(as_signed(head_.load(X) - tail) <= 0))
          return false;
      } while (ATOMIC_QUEUE_UNLIKELY(!tail_.compare_exchange_weak(
          tail, tail + 1, X, X)));  // This loop is not FIFO.
    }
  ...
  ATOMIC_QUEUE_INLINE void push(T&& element) noexcept {
    ...
    if (Derived::spsc_) {
      ...
      head = head_.fetch_add(1, memory_order);  // FIFO and total order on Intel
                                                // regardless, as of 2019.
    }
    downcast().do_push(std::forward<T>(element), head);
  }
  ...
  ATOMIC_QUEUE_INLINE auto pop() noexcept {
    ...
    if (Derived::spsc_) {
      ...
      constexpr auto memory_order = Derived::total_order_
                                        ? std::memory_order_seq_cst
                                        : std::memory_order_relaxed;
      tail = tail_.fetch_add(1, memory_order);  // FIFO and total order on Intel
                                                // regardless, as of 2019.
    }
    return downcast().do_pop(tail);
  }

  ATOMIC_QUEUE_INLINE bool was_empty() const noexcept { return !was_size(); }
  ...
  ATOMIC_QUEUE_INLINE unsigned was_size() const noexcept {
    // tail_ can be greater than head_ because of consumers doing pop, rather
    // that try_pop, when the queue is empty.
